...
class I2C(MMIOMemoryRegion):
                ...
                cr1 = utils.replace_bit(
                    ...
                    claripy.If(
                        ...
                        utils.generate_symbolic(
                            state, f"SYMBIT_{i2c.name}_STOP", size=1
                        ),
                    ),
                )
            ...
            if force or not state.solver.unique(sr1[cls.AF.bit]):
                ...
                if value is None:
                    value = claripy.If(
                        sr1[cls.AF.bit] == 1,
                        sr1[cls.AF.bit],
                        utils.generate_symbolic(state, f"SYMBIT_{i2c.name}_AF", size=1),
                    )
            ...
            if force or not state.solver.unique(sr1[cls.ARLO.bit]):
                ...
                if value is None:
                    value = claripy.If(
                        ...
                        sr1[cls.ARLO.bit],
                        utils.generate_symbolic(
                            state, f"SYMBIT_{i2c.name}_ARLO", size=1
                        ),
                    )
            ...
            if force or not state.solver.unique(sr1[cls.TXE.bit]):
                ...
                if value is None:
                    value = claripy.If(
                        ...
                        sr1[cls.TXE.bit],
                        utils.generate_symbolic(
                            state, f"SYMBIT_{i2c.name}_TXE", size=1
                        ),
                    )
            ...
            if force or not state.solver.unique(sr1[cls.ADD10.bit]):
                ...
                if value is None:
                    value = claripy.If(
                        ...
                        sr1[cls.ADD10.bit],
                        utils.generate_symbolic(
                            state, f"SYMBIT_{i2c.name}_ADD10", size=1
                        ),
                    )
            ...
            if force or not state.solver.unique(sr1[cls.BTF.bit]):
                ...
                if value is None:
                    value = claripy.If(
                        ...
                        sr1[cls.BTF.bit],
                        utils.generate_symbolic(
                            state, f"SYMBIT_{i2c.name}_BTF", size=1
                        ),
                    )
            ...
            if force or not state.solver.unique(sr1[cls.ADDR.bit]):
                ...
                if value is None:
                    value = claripy.If(
                        ...
                        sr1[cls.ADDR.bit],
                        utils.generate_symbolic(
                            state, f"SYMBIT_{i2c.name}_ADDR", size=1
                        ),
                    )
            ...
            if force or not state.solver.unique(sr1[cls.SB.bit]):
                ...
                if value is None:
                    value = claripy.If(
                        sr1[cls.SB.bit] == 1,
                        sr1[cls.SB.bit],
                        utils.generate_symbolic(state, f"SYMBIT_{i2c.name}_SB", size=1),
                    )
    ...
    SYMBOLIC_BITS = {
        I2C_CR1.OFFSET: [I2C_CR1.STOP.bit],
        I2C_SR1.OFFSET: [
            I2C_SR1.SB.bit,
            I2C_SR1.ADDR.bit,
            I2C_SR1.BTF.bit,
    ...
    def post_read(self, state):
        ...
        readout_value = self.mask_post_read(offset, readout_value)

        def annotate_symbolic_bits(bv):
            for idx in I2C.SYMBOLIC_BITS.get(offset, []):
                sym_bit = claripy.BVS(f"SYMBIT_{self.name}_{hex(offset)}[{idx}]", 1)
                state.add_constraints(sym_bit == bv[idx])

                bv = utils.replace_bit(bv, idx, sym_bit)

            return bv

        state.inspect.mem_read_expr = annotate_symbolic_bits(readout_value)
