            ...
            case self.CR1_OFFSET:
                ...
                if not state.solver.satisfiable(extra_constraints=[val[8] == 0]):
                    ...
                    utils.set_symbolic(
                        ...
                        self.start + self.SR2_OFFSET,
                        I2C.SR2_TRA_MASK,
                        f"I2C1_{self.SR2_OFFSET:#x}_TRA",
                    )
...
def precondition(proj, state, I2C1):  # MODIFY:
    ...
    # cr2 = utils.load(state, I2C1.CR2)
    # if state.solver.satisfiable(extra_constraints=[cr2[11] == 0]):
    #     return False
...
def main():
    ...
    while True:
        ...
        sram_dump = stm32.read_memory(
            RAM.start, size=4, num_words=RAM.size // 4, raw=True
        )
        try:
        ...
        try:
            state.memory.store(
                VECTOR_TABLE.start, vector_table_dump
            )  # Vector Table Alias
            logger.info(f"Mapped Flash alias at {VECTOR_TABLE.start:#x} (Vector Table)")
        except Exception as e:
            logger.warning(
                f"Failed to map Vector Table at {VECTOR_TABLE.start:#x}: {e}"
            )
        ...
        if precondition(proj, state, I2C1):
            logger.info("Precondition met")
            break
        else:
            logger.info("Precondition not met, resume avatar execution")
