...
static uint32_t SDIO_S_SendCMDWithOutDAT(SDIO_TypeDef *sdio_t, uint32_t cmd_u32,
                                         uint32_t cmdArg_u32);
static void SDIO_S_LowLevelRegisterInit(SDIO_TypeDef *sdio_t, uint32_t sdioFreq_u32,
...
/**************************************************************************/ /**
                                                                              * @brief General
                                                                              *initialization
                                                                              *function for the SDIO
                                                                              *host and for SD cards
                                                                              *
                                                                              * @Inputs:
                                                                              * 		 SDIO_TypeDef
                                                                              **sdio_t: address of
                                                                              ...
                                                                              *main clock source
                                                                              *****************************************************************************/
void SDIO_Init(SDIO_TypeDef *sdio_t, uint32_t sdioFreq_u32, CMU_Clock_TypeDef mainClock_t) {
...
/**************************************************************************/ /**
                                                                              ...
                                                                              *shall be 512byte
                                                                              *****************************************************************************/
void SDIO_ReadSingleBlock(SDIO_TypeDef *sdio_t, uint32_t SD_origin_u32,
                          uint32_t *destination_pu32) {
    SDIO_S_TimeoutSettingonDATLine(sdio_t);

    volatile uint32_t *localbuffptr_pu32;
    ...
    tmpReg_u32 |= SDIO_TFRMODE_MULTSINGBLKSEL_SINGLEBLK |
                  ...
                  | SDIO_TFRMODE_AUTOCMDEN_ACMDDISABLED | SDIO_TFRMODE_BLKCNTEN_DISABLE |
                  SDIO_TFRMODE_DMAEN_DISABLE

                  // 5. Set Command Reg
                  | SDIO_S_SetCommandResponseType(CMD17) | SDIO_TFRMODE_CMDCRCCHKEN_DISABLE |
                  SDIO_TFRMODE_CMDINDXCHKEN_DISABLE | SDIO_TFRMODE_DATPRESSEL_DATA |
                  SDIO_TFRMODE_CMDTYPE_NORMAL | (CMD17 << _SDIO_TFRMODE_CMDINDEX_SHIFT);
...
/**************************************************************************/ /**
                                                                              ...
                                                                              * 		 SD_dest_u32:
                                                                              *address of the SD
                                                                              *card block, where the
                                                                              *data will be copied
                                                                              *
                                                                              * 		 *origin_pu32:
                                                                              *address of the data,
                                                                              *from the data will be
                                                                              *transfered to SD
                                                                              *card. It shall be
                                                                              *512byte
                                                                              *****************************************************************************/
void SDIO_WriteSingleBlock(SDIO_TypeDef *sdio_t, uint32_t SD_dest_u32, uint32_t *origin_pu32) {
    ...
    tmpReg_u32 |= SDIO_TFRMODE_MULTSINGBLKSEL_SINGLEBLK |
                  ...
                  // 5. Set Command Reg
                  | SDIO_S_SetCommandResponseType(CMD24) | SDIO_TFRMODE_CMDCRCCHKEN_DISABLE |
                  SDIO_TFRMODE_CMDINDXCHKEN_DISABLE | SDIO_TFRMODE_DATPRESSEL_DATA |
                  SDIO_TFRMODE_CMDTYPE_NORMAL | (CMD24 << _SDIO_TFRMODE_CMDINDEX_SHIFT);
...
/**************************************************************************/ /**
                                                                              ...
                                                                              *argument
                                                                              *****************************************************************************/
uint32_t SDIO_S_SendCMDWithOutDAT(SDIO_TypeDef *sdio_t, uint32_t cmd_u32, uint32_t cmdArg_u32) {
    uint32_t tmpReg_u32;
...
static uint32_t SDIO_S_SetCommandSpecificCrcAndIndexCheck(uint32_t cmd_u32) {
    ...
    if (cmd_u32 == CMD8) {
        retVal_u32 = (SDIO_TFRMODE_CMDCRCCHKEN_ENABLE | SDIO_TFRMODE_CMDINDXCHKEN_ENABLE);
    }
...
static uint32_t SDIO_S_SetCommandResponseType(uint32_t cmd_u32) {
    ...
    /*R1 response*/
    else if (cmd_u32 == CMD55 || cmd_u32 == CMD58 || cmd_u32 == CMD17 || cmd_u32 == CMD24 ||
             cmd_u32 == ACMD6 || cmd_u32 == CMD13) {
    ...
    else if (cmd_u32 == CMD3 || cmd_u32 == CMD5) {
        return SDIO_TFRMODE_RESPTYPESEL_RESP48;
    }
    /*R7 response*/
...
/**************************************************************************/ /**
                                                                              ...
                                                                              * 		  SD_Host_Controller_Simplified_Specification_Ver3.00
                                                                              *Chapter2.x
                                                                              *
                                                                              * @Inputs:
                                                                              * 		 SDIO_TypeDef
...
static void SDIO_S_LowLevelRegisterInit(SDIO_TypeDef *sdio_t, uint32_t sdioFreq_u32,
    ...
    // sdio_t->ROUTELOC0 |= SDIO_ROUTELOC0_DATLOC_LOC0 | SDIO_ROUTELOC0_CLKLOC_LOC0 |
    // SDIO_ROUTELOC1_CMDLOC_LOC0;
    sdio_t->ROUTELOC0 =
        SDIO_ROUTELOC0_DATLOC_LOC0 | SDIO_ROUTELOC0_CDLOC_LOC2 | SDIO_ROUTELOC0_CLKLOC_LOC0;

    sdio_t->ROUTELOC1 = SDIO_ROUTELOC1_CMDLOC_LOC0; // cmd location is right

    sdio_t->ROUTEPEN = SDIO_ROUTEPEN_CLKPEN | SDIO_ROUTEPEN_CMDPEN | SDIO_ROUTEPEN_D0PEN |
                       SDIO_ROUTEPEN_D1PEN | SDIO_ROUTEPEN_D2PEN | SDIO_ROUTEPEN_D3PEN;
    /*
     * General SDIO register adjustment
     */
    sdio_t->CTRL = (0 << _SDIO_CTRL_ITAPDLYEN_SHIFT) | (0 << _SDIO_CTRL_ITAPDLYSEL_SHIFT) |
                   (0 << _SDIO_CTRL_ITAPCHGWIN_SHIFT) | (1 << _SDIO_CTRL_OTAPDLYEN_SHIFT) |
                   (8 << _SDIO_CTRL_OTAPDLYSEL_SHIFT);

    sdio_t->CFG0 = (0x20 << _SDIO_CFG0_TUNINGCNT_SHIFT) | (0x30 << _SDIO_CFG0_TOUTCLKFREQ_SHIFT) |
                   (1 << _SDIO_CFG0_TOUTCLKUNIT_SHIFT) | (0xD0 << _SDIO_CFG0_BASECLKFREQ_SHIFT) |
                   (SDIO_CFG0_MAXBLKLEN_1024B) | (1 << _SDIO_CFG0_C8BITSUP_SHIFT) |
                   (1 << _SDIO_CFG0_CADMA2SUP_SHIFT) | (1 << _SDIO_CFG0_CHSSUP_SHIFT) |
                   (1 << _SDIO_CFG0_CSDMASUP_SHIFT) | (1 << _SDIO_CFG0_CSUSPRESSUP_SHIFT) |
                   (1 << _SDIO_CFG0_C3P3VSUP_SHIFT) | (1 << _SDIO_CFG0_C3P0VSUP_SHIFT) |
                   (1 << _SDIO_CFG0_C1P8VSUP_SHIFT);

    sdio_t->CFG1 = (0 << _SDIO_CFG1_ASYNCINTRSUP_SHIFT) | (SDIO_CFG1_SLOTTYPE_EMSDSLOT) |
                   (1 << _SDIO_CFG1_CSDR50SUP_SHIFT) | (1 << _SDIO_CFG1_CSDR104SUP_SHIFT) |
                   (1 << _SDIO_CFG1_CDDR50SUP_SHIFT) | (1 << _SDIO_CFG1_CDRVASUP_SHIFT) |
                   (1 << _SDIO_CFG1_CDRVCSUP_SHIFT) | (1 << _SDIO_CFG1_CDRVDSUP_SHIFT) |
                   (1 << _SDIO_CFG1_RETUNTMRCTL_SHIFT) | (1 << _SDIO_CFG1_TUNSDR50_SHIFT) |
                   (0 << _SDIO_CFG1_RETUNMODES_SHIFT) | (1 << _SDIO_CFG1_SPISUP_SHIFT) |
                   (1 << _SDIO_CFG1_ASYNCWKUPEN_SHIFT);

    sdio_t->CFGPRESETVAL0 =
        (0 << _SDIO_CFGPRESETVAL0_INITSDCLKFREQ_SHIFT) |
        (0 << _SDIO_CFGPRESETVAL0_INITCLKGENEN_SHIFT) | (0 << _SDIO_CFGPRESETVAL0_INITDRVST_SHIFT) |
        (0x4 << _SDIO_CFGPRESETVAL0_DSPSDCLKFREQ_SHIFT) |
        (0 << _SDIO_CFGPRESETVAL0_DSPCLKGENEN_SHIFT) | (0x3 << _SDIO_CFGPRESETVAL0_DSPDRVST_SHIFT);
    ...
    sdio_t->IFENC = SDIO_IFENC_CMDCOMEN | SDIO_IFENC_TRANCOMEN | SDIO_IFENC_BUFWRRDYEN |
                    SDIO_IFENC_BUFRDRDYEN | SDIO_IFENC_CMDCRCERREN | SDIO_IFENC_CMDINDEXERREN |
                    SDIO_IFENC_DATTOUTERREN | SDIO_IFENC_DATCRCERREN;

    {
        ...
        sdio_t->CLOCKCTRL = ((divisor_u32 << _SDIO_CLOCKCTRL_SDCLKFREQSEL_SHIFT)) |
                            (SDIO_CLOCKCTRL_INTCLKEN) | (SDIO_CLOCKCTRL_SDCLKEN);
    }

    sdio_t->HOSTCTRL1 = (SDIO_HOSTCTRL1_SDBUSVOLTSEL_3P0V) | (SDIO_HOSTCTRL1_SDBUSPOWER) |
    ...
    if (SDIO_ActCardState_st.F8_u1 == 1) {
        ...
        if (tempVar_u32 != sdio_t->RESP0) {
            ...
            while (!(sdio_t->RESP0 & (1 << _ACMD41_RESPONSE_BUSY_BIT_SHIFT))) {
                ...
                SDIO_S_SendCMDWithOutDAT(sdio_t, CMD55, RCA_DEFAULT << _CMD55_RCA_SHIFT);
                SDIO_S_SendCMDWithOutDAT(sdio_t, ACMD41,
                                         1 << _ACMD41_VW_31_32_SHIFT | 1 << _ACMD41_HCS_SHIFT |
                                             1 << _ACMD41_XPC_SHIFT | 1 << _ACMD41_S18R_SHIFT);
                SDIO_ActCardState_st.CCS_u1 = ((sdio_t->RESP0 & 1 << 30) >> 30);
                SDIO_ActCardState_st.S18A_u1 = ((sdio_t->RESP0 & 1 << 24) >> 24);
            }
            // SDIO_ActCardState_st.CCS_u1 = 1;
...
/**************************************************************************/ /**
                                                                              ...
                                                                              *mapped register
                                                                              *
                                                                              *		 SDIO_ActCardState_st.cardRCA_u16:
                                                                              *actual SD card
                                                                              *related relative
                                                                              *		 								   address
                                                                              *
...
static void SDIO_S_CardStdbyToTrsfrMode(SDIO_TypeDef *sdio_t) {
    ...
    SDIO_S_SendCMDWithOutDAT(sdio_t, CMD13, SDIO_ActCardState_st.cardRCA_u16 << _CMD13_RCA_SHIFT);
    // 2. Select the actual card to active
    SDIO_S_SendCMDWithOutDAT(sdio_t, CMD7, SDIO_ActCardState_st.cardRCA_u16 << _CMD7_RCA_SHIFT);
    // 3. Change to 4bit mode
    SDIO_S_SendCMDWithOutDAT(sdio_t, CMD55, SDIO_ActCardState_st.cardRCA_u16 << _CMD55_RCA_SHIFT);
...
/**************************************************************************/ /**
                                                                              ...
                                                                              *type of the card
                                                                              *****************************************************************************/
uint8_t SDIO_GetActCardStateType(void) { return ((uint8_t)(SDIO_ActCardState_st.cardType_u3)); }
